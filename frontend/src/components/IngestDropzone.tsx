...
const STAGES: Array<{ key: Stage; title: string; desc: string }> = [
  ...
  { key: "parse", title: "Parse", desc: "Extracting structure and prose." },
  {
    key: "compile",
    title: "Compile",
    desc: "Folding new facts into the wiki.",
  },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <label
        ...
        style={{
          background: hover ? "var(--color-surface)" : "var(--color-canvas)",
      ...
      >
        ...
        <span className="text-[13px] text-fg-muted font-sans text-center">
          ...
          — accepts{" "}
          <code className="font-mono bg-sand text-fg px-1 rounded">.md</code>
        </span>
        <input
          type="file"
          accept=".md"
        ...
        >
          ...
          <ol className="grid grid-cols-4 gap-2.5">
            {STAGES.map((s, i) => {
              ...
              const active =
                i === currentIdx && job.status !== "done" && !failed;
              const complete =
                i < currentIdx || (i === currentIdx && job.status === "done");
              const color =
                failed && reached
                  ? "var(--color-error-crimson)"
                  : complete
                    ...
                    {active && !failed && (
                      <span
                        ...
                        style={{
                          ...
                          animation: "ingest-shimmer 1.8s linear infinite",
                        }}
                      />
                    )}
                  </span>
